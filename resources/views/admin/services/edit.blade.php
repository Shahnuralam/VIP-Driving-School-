                ...
                <div class="col-md-8">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description" id="description" rows="5" 
                                  class="form-control summernote @error('description') is-invalid @enderror">{{ old('description', $service->description) }}</textarea>
                        @error('description')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="col-md-4">
                    ...
                    <div class="form-group">
                        ...
                        <select name="location_id" id="location_id" class="form-control @error('location_id') is-invalid @enderror">
                            ...
                            @foreach($locations as $location)
                                <option value="{{ $location->id }}" {{ old('location_id', $service->location_id) == $location->id ? 'selected' : '' }}>
                                    {{ $location->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select name="transmission_type" id="transmission_type" class="form-control @error('transmission_type') is-invalid @enderror" required>
                            <option value="auto" {{ old('transmission_type', $service->transmission_type) == 'auto' ? 'selected' : '' }}>Automatic</option>
                            <option value="manual" {{ old('transmission_type', $service->transmission_type) == 'manual' ? 'selected' : '' }}>Manual</option>
                            <option value="both" {{ old('transmission_type', $service->transmission_type) == 'both' ? 'selected' : '' }}>Both</option>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        @if($service->image)
                            <div class="mb-2">
                                <img src="{{ asset('storage/' . $service->image) }}" alt="Current image" class="img-thumbnail" style="max-width: 150px;">
                            </div>
                        @endif
